...
class Upload extends BaseController
{
    ...
    public function upload_dokumen_d()
    {
        ...
        if (!$q) {
            ...
            gagal($url, 'Id Kelurahan tidak ditemukan.');
        }
        ...
        $randomname = str_replace(" ", "-", str_replace("/", "", str_replace(".", "", str_replace(",", "", time() . '_'  . '_' . strtolower($kelurahan)))));

        if ($file['error'] == 0) {
            $size = $file['size'];

            if ($size > 6000000) {
                gagal($url, 'Ukuran file maksimal 5 MB.');
            }
            ...
            if (settings('online')) {
                $dir = '/home/u1733924/public_html/pileg/files/dokumen_d/';
            }
            ...
            if (!move_uploaded_file($file['tmp_name'], $upload)) {
                ...
                if ($q['dokumen_d'] !== 'file-not-found.jpg') {
                    if (!unlink($dir . $q['dokumen_d'])) {
                        gagal($url, 'File lama gagal dihapus.');
                    }
                }

                $q['dokumen_d'] = $randomname . '.' . $exe;
